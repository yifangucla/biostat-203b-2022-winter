...
labevents_tble <- labevents_tble %>% 
  ...
  rename("charttime_labevent" = "charttime",
         "creatinine" = "lab50912",
         "potassium" = "lab50971",
         "sodium" = "lab50983",
         "chloride" = "lab50902",
...
chartevents_tble <- chartevents_tble %>% 
  ...
  pivot_wider(names_from = itemid, names_prefix = "chart", 
              values_from = valuenum) %>%
  rename("charttime_chartevent" = "charttime",
         "heart_rate" = "chart220045",
...
icu_cohort <- icustays_tble %>%
  ...
  filter((age_hadm) > 18) %>% 
  mutate(hadm_death = ifelse(is.na(dod), NA, 
                             TIMESTAMP_DIFF(dod, date(admittime), DAY))) %>% 
  mutate(thirty_day_mort = hadm_death <= 30) %>% 
  mutate(thirty_day = ifelse(is.na(thirty_day_mort), F, thirty_day_mort)) %>%
  collect() %>%
  print(width = Inf)
```

```{r}
write_rds(icu_cohort, "~/homework-203B-2022-Winter/hw3/mimiciv_shiny/icu_cohort.rds")
```
